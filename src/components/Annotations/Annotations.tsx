...
export interface AnnotationsRef {
  getAnnotations: () => AnnotationEntry[];
  setAnnotations: (entries: AnnotationEntry[]) => void;
...
export const Annotations = forwardRef<AnnotationsRef, AnnotationsProps>(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  function Annotations({ viewerRef, active, onClose, className }, ref) {
    ...
    // Keep a ref to current annotations for imperative access
    const annotationsRef = useRef<AnnotationEntry[]>([]);
    useEffect(() => {
      annotationsRef.current = annotations;
    }, [annotations]);

    // ── Imperative handle for Viewpoints integration ──────────
    useImperativeHandle(ref, () => ({
      getAnnotations() {
        return annotationsRef.current;
      },
      setAnnotations(entries: AnnotationEntry[]) {
    ...
    const drawAnnotation = useCallback((ctx: CanvasRenderingContext2D, ann: AnnotationEntry) => {
      switch (ann.type) {
        case 'freehand':
    ...
    }, [drawFreehand, drawCloud, drawArrow, drawText]);
    ...
    // Show canvas overlay when there are annotations OR when active (drawing mode)
    const hasAnnotations = annotations.length > 0;
    const showCanvas = active || hasAnnotations;
    ...
    return (
      ...
      >
        ...
        {/* Drawing canvas — stays visible as long as annotations exist */}
        <canvas
          ref={canvasRef}
